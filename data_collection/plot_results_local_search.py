...
def threshold_plot(
    ...
    dir = "LatticeDecoder.jl/results/figures/" + "".join(file.split("/")[:-1])
    print(dir)
    os.makedirs(dir, exist_ok=True)

    data = sinter.stats_from_csv_files(
        f"LatticeDecoder.jl/results/{file:s}.csv"
    )
        ...
        for ls in local_search:
            for ls_lll in local_search_lll:
                for sch in schedules:
                    ...
                    pl.tight_layout(fig)
                    fig.savefig(
                        f"LatticeDecoder.jl/results/figures/{file}_{dec}_{ls}_{ls_lll}_{sch}.pdf"
                    )
                    fig.savefig(
                        f"LatticeDecoder.jl/results/figures/{file}_{dec}_{ls}_{ls_lll}_{sch}.png"
                    )
                    plt.show()
        ...
        for ls in local_search:
            for ls_lll in local_search_lll:
                for sch in schedules:
                    ...
                    pl.tight_layout(fig)
                    fig.savefig(
                        f"LatticeDecoder.jl/results/figures/{file}_scaling_{dec}_{ls}_{ls_lll}_{sch}.pdf"
                    )
                    fig.savefig(
                        f"LatticeDecoder.jl/results/figures/{file}_scaling_{dec}_{ls}_{ls_lll}_{sch}.png"
                    )
                    plt.show()
...
paths =[
    ...
    "quantum_rep_code/balanced_first_reduced_check_matrix_ls_sd",
    ]
schedules = ["parallel", "serial"]
decoders = ["nearest"]
local_search = [True]
local_search_lll = [False, True]
