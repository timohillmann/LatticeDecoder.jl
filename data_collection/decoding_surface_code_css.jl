...
function css_surface_code_problem(d::Int, basis::AbstractString)
    ...
    Mqq = Matrix{Float64}(M[1:half, 1:half])
    Mpp = Matrix{Float64}(M[(half + 1):end, (half + 1):end])

    if basis == "X"
        return (H = Mqq, G = inv(Mpp))
    elseif basis == "Z"
        return (H = Mpp, G = -inv(Mqq))
    else
        error("Unknown CSS basis: $(basis)")
    end
end

@everywhere function run_surface_code_css_samples(;
...
function run_surface_code_css_experiment!(
    path::AbstractString;
    param_ranges::Dict,
    n_samples::Int,
    repeats::Int,
    ...
    for _ in 1:repeats
        for params in parameter_grid(param_ranges)
            ...
            for σ in params[:sigmas]
                ...
                errors = run_surface_code_css_samples(;
                    H,
                    G,
                    logical_check,
                    σ,
