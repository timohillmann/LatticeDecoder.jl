...
def snr_db(sigma: float):

    return -10 * np.log10(2 * np.pi * np.exp(1) * sigma**2)
        ...
        for ls in local_search:
            for sch in schedules:
                ...
                sinter.plot_error_rate(
                    ...
                    x_func=lambda stat: x_func(stat.json_metadata["sigma"]),
        ...
        for ls in local_search:
            for sch in schedules:
                ...
                sinter.plot_error_rate(
                    ax=ax,
                    stats=data,
                    x_func=lambda stat: stat.json_metadata["n"],
