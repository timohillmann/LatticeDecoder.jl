...
@everywhere function qec_sample(
    ...
    lsd::LatticeDecoder.LatticeStatisticsDecoding,
...
@everywhere function qec_experiment(logical, H, G, sigmas, n_samples, order, local_search, schedule, reduced_basis, iterations, decoder, decoding_style, search_radius)
    results = []
    tg = initialize_tanner_graph(H)
    lsd = LatticeStatisticsDecoding(order, G, reduced_basis)
    results = [qec_sample(tg, lsd, H, G, logical, σ, n_samples; schedule, iterations, decoder, decoding_style, search_radius) for σ in sigmas]
    return results
end
...
path = "results/quantum_rep_code/results.csv"
for r = 1:1
