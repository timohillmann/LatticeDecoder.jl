...
tgq = initialize_tanner_graph_quant(H);
...
bp_nearest = run_belief_propagation!(tg, y, σ, 10, "nearest");
bp_quant = run_belief_propagation!(tgq, y, σ, 10);

dec_quant = hard_decision(bp_quant, H);
dec_nearest = hard_decision(bp_nearest, H);

println("Number of symbol errors quantized: ", count_symbol_errors(dec_quant, b))
println("Number of symbol errors nearest: ", count_symbol_errors(dec_nearest, b))
...
@everywhere function random_encoding_experiment(H, σ, max_iter, samples, decoder)
    ...
    G = generator_matrix(H)
    # tg = initialize_tanner_graph(H)
    tg = initialize_tanner_graph_quant(H);
    ...
    errors = @distributed (+) for i = 1:samples
        ...
        y = encode(b, G)
        y .+= sample_error(σ, n)
        bp_result = run_belief_propagation_parallel!(tg, y, σ, max_iter);
