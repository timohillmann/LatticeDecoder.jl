...
mutable struct LDLCDecoder{TG,CM,DM,PM,MA}
    tg::TG
    schedule::Symbol
    ...
    channel_messages::CM
    damping_messages::DM
    posterior_message::PM
    m_gaussian_allocs::MA
end
...
"""
    LDLCDecoder(tg::TannerGraphQuant; schedule=:parallel, sigma=1.0,
                max_iterations=25)

Wrap a discretized-PDF Tanner graph in the common mutable decoder API. The
graph selects the quantized algorithm, so `algorithm` must be `:quantized`.
Supported schedules are `:parallel`, `:serial`/`:serial_vertical`, and
`:serial_horizontal`.
"""
function LDLCDecoder(
    tg::TannerGraphQuant;
    ...
    M === nothing || throw(ArgumentError("M-Gaussian decoding is not supported by TannerGraphQuant"))
    decoder_schedule = _normalize_decoder_schedule(schedule)
    decoder_algorithm = _normalize_decoder_algorithm(algorithm, nothing)
    _validate_quantized_decoder_config(decoder_schedule, decoder_algorithm)
    decoder_sigma = _validate_quantized_sigma(sigma)
    decoder_iterations = _validate_quantized_iterations(max_iterations)
    return LDLCDecoder(
        tg,
        decoder_schedule,
        decoder_algorithm,
        ...
        decoder_iterations,
        1.5,
        LatticeDecoder.LSD_DEFAULT_BETA,
        LatticeDecoder.LSD_W_MIN,
        0.0,
        0.0,
        nothing,
        nothing,
...
function _validate_quantized_decoder_config(schedule::Symbol, algorithm::Symbol)
    schedule in (:parallel, :serial, :serial_vertical, :serial_horizontal) ||
        throw(ArgumentError("Unsupported quantized schedule $(schedule). Choose :parallel, :serial, :serial_vertical, or :serial_horizontal."))
    algorithm == :quantized ||
        throw(ArgumentError("TannerGraphQuant requires algorithm :quantized."))
    return nothing
end

function _validate_quantized_decoder_config(schedule, algorithm)
    throw(ArgumentError("TannerGraphQuant requires a symbolic schedule and algorithm :quantized."))
end

function _validate_quantized_sigma(sigma::Real)
    value = Float64(sigma)
...
function _validate_quantized_iterations(max_iterations::Integer)
    max_iterations >= 0 || throw(ArgumentError("max_iterations must be nonnegative"))
    return Int64(max_iterations)
end
...
function run_decoder!(dec::LDLCDecoder{<:TannerGraphQuant}, message::Vector{Float64})
    _validate_quantized_decoder_config(dec.schedule, dec.algorithm)
    dec.sigma = _validate_quantized_sigma(dec.sigma)
    dec.max_iterations = _validate_quantized_iterations(dec.max_iterations)

    if dec.schedule == :parallel
        return _run_sommer_decoder!(dec.tg, message, dec.sigma, dec.max_iterations)
    elseif dec.schedule == :serial_horizontal
        return _run_sommer_horizontal_decoder!(dec.tg, message, dec.sigma, dec.max_iterations)
    else
        return _run_sommer_serial_decoder!(
            dec.tg,
            message,
            dec.sigma,
            dec.max_iterations,
...
function run_decoder_parallel!(dec::LDLCDecoder{<:TannerGraphQuant}, message::Vector{Float64}, sigma::Float64, max_iter::Int64)
    dec.schedule = :parallel
    dec.sigma = _validate_quantized_sigma(sigma)
    dec.max_iterations = _validate_quantized_iterations(max_iter)
    return run_decoder!(dec, message)
end
...
function run_decoder_serial!(dec::LDLCDecoder{<:TannerGraphQuant}, message::Vector{Float64}, sigma::Float64, max_iter::Int64)
    dec.schedule = :serial
    dec.sigma = _validate_quantized_sigma(sigma)
    dec.max_iterations = _validate_quantized_iterations(max_iter)
    return run_decoder!(dec, message)
