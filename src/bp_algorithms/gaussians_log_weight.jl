...
A Gaussian component represented as

    exp(log_weight) * Normal(mean, var),

where `Normal(mean, var)` is a normalized Gaussian density.

For a periodic check-to-variable message, the component represents the
periodic family

    sum(
        exp(log_weight) *
        Normal(x; mean - b / period, var)
        for b in integers
    ).

The `period` field is only meaningful for periodic check-to-variable
messages. It is unused for channel and variable-to-check messages.

Invariants:

- `var > 0`
- `period != 0` whenever the message is interpreted periodically
"""
