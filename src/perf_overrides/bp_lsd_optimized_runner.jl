const OVERLAY_ACTIVE = Ref(false)

function variable_node_iterations_lsd_optimized!(tg::LD.TannerGraph)
    @inbounds for vn_idx in 1:tg.nv
        lsd_variable_node_messages_optimized!(tg, vn_idx)
    end
    return nothing
end

function decision_step_lsd_optimized!(tg::LD.TannerGraph)
    @inbounds for vn_idx in 1:tg.nv
        lsd_variable_node_decision_optimized!(tg, vn_idx)
    end
    return nothing
end

function run_belief_propagation_lsd_optimized!(
    tg::LD.TannerGraph,
    message::Vector{Float64},
    σ::Float64,
    max_iter::Int64;
    search_interval::Float64=1.5,
    threaded::Bool=false,
)
    if threaded
        throw(ArgumentError("Internal threading is disabled. Use outer-loop threading instead and call with threaded=false."))
    end

    tg.search_interval = search_interval
    LD.initialize_messages!(tg, message, σ)

    @inbounds for _ in 1:max_iter
        LD.check_node_iterations!(tg)
        variable_node_iterations_lsd_optimized!(tg)
    end

    decision_step_lsd_optimized!(tg)
    return tg.bp_result
end

function activate_lsd_overlay!()
    if OVERLAY_ACTIVE[]
        return nothing
    end

    overlay_mod = @__MODULE__
    @eval LD begin
        function lsd_variable_node_messages!(tg::TannerGraph, vn_idx::Int64)
            return $overlay_mod.lsd_variable_node_messages_optimized!(tg, vn_idx)
        end

        function _lsd_variable_node_decision!(bp_result::Vector{Float64}, tg::TannerGraph, vn_idx::Int64)
            return $overlay_mod.lsd_variable_node_decision_optimized!(bp_result, tg, vn_idx)
        end

        function _lsd_variable_node_decision!(tg::TannerGraph, vn_idx::Int64)
            return $overlay_mod.lsd_variable_node_decision_optimized!(tg, vn_idx)
        end
    end

    OVERLAY_ACTIVE[] = true
    return nothing
end
